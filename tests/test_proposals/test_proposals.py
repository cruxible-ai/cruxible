...
def test_a_card_bearing_submission_leaves_no_unreachable_proposal_commit(
    ...
    """Publish the evaluated snapshot directly, without an intermediate authored commit."""
...
def test_resubmitting_one_ref_keeps_active_candidates_without_chaining(
    tmp_path: Path,
) -> None:
    """Each active candidate is rooted independently at the accepted base."""
    ...
    assert all(ledger.parent_of(tip) == accepted_head for tip in tips)
    assert all(not ledger.is_ancestor(earlier, tips[-1]) for earlier in tips[:-1])
    ledger._git(["reflog", "expire", "--expire=now", "--all"])
    ledger._git(["gc", "--prune=now"])
    assert all(ledger.object_exists(tip) for tip in tips)
...
def test_proposal_service_guard_contains_a_direct_advertiser_failure(tmp_path: Path) -> None:
    ...
    service = ProposalService(
        ...
        note_index_provider=instance.proposal_note_index,
