...
from tests.test_authoring.test_authoring_change_set_intents import _change_set, _claim, _coordinator
...
@pytest.mark.parametrize("publication", ["allowed", "unauthorized", "head_moved"])
def test_prepared_submission_preserves_authorization_and_publication_fence(
    world, monkeypatch, publication
):
    instance = world[0]
    adapter = instance.prepared_evaluations
    with adapter.scope() as scope:
        ...
        service = instance.proposal_service()
        authorized = []
        original_main = service.transport.read_main

        def authorize(current, tree):
            authorized.append(current)
            assert current == args["current"]
            assert tree == instance.immutable_tree_at(current.git_oid)
            assert not scope.consumed
            if publication == "unauthorized":
                raise proposals.ProposalAdmissionError("fresh mandate refusal")

        def read_main():
            if publication == "head_moved" and scope.consumed:
                return "f" * len(args["current"].git_oid)
            return original_main()

        def unexpected_evaluation(**kwargs):
            pytest.fail("an eligible handoff should not repeat evaluation")

        monkeypatch.setattr(service.transport, "read_main", read_main)
        monkeypatch.setattr(proposals, "evaluate_proposal_tree", unexpected_evaluation)
        submit_args = dict(
            actor=args["actor"],
            request=args["request"],
            candidate_tree=args["tree"],
        ...
        if publication == "allowed":
            result = service.submit(**submit_args)
            assert result.candidate == world[-1].evaluation.candidate
            assert adapter.status()["reused"] == 1
        else:
            error = (
                proposals.ProposalHeadMovedError
                if publication == "head_moved"
                else proposals.ProposalAdmissionError
            )
            with pytest.raises(error):
                service.submit(**submit_args)
            assert service.transport.read_proposal_ref(args["request"].target_ref) is None
