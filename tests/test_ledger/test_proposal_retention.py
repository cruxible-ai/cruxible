...
from cruxible_client.contracts.errors import ProposalContentUnavailable
...
def test_closed_candidates_can_be_collected_while_accepted_and_active_work_survives(
    ...
    assert not instance._ledger.object_exists(closed.admission.candidate_commit_oid)
    ...
    with pytest.raises(ProposalContentUnavailable):
    ...
    assert not any(ref.startswith("refs/settled/") for ref in reopened._ledger.mirror_refs())
