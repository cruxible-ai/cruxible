...
def test_nested_union_preserves_exact_tagless_v1_and_accepts_strict_tagged_v2(
    tmp_path: Path,
) -> None:
    previous = {
    ...
    assert isinstance(result.source_observations[0], PlaybillNextSourceObservationV2)  # type: ignore[index]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda value: value.update({"tag": "playbill-next-source-observation-v3"}),
...
def test_nested_v2_refuses_unknown_fields_and_unverified_or_incomplete_occurrences(
    ...
    with pytest.raises(ValidationError):
        PlaybillNextSourceObservationV2.model_validate(candidate)
