...
def test_every_command_registered_on_a_group_is_in_the_lazy_cli_map() -> None:
    ...
    assert len(groups) == 18, f"expected 18 Playbill/host groups, found {len(groups)}"
...
def test_every_command_defined_in_the_commands_package_is_reachable() -> None:
    ...
    assert len(leaf_claims) == 64, (
        f"expected 64 Playbill/host leaf commands, found {len(leaf_claims)}"
    )
