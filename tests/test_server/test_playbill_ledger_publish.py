...
def _state():
    return LedgerMirrorStateV1(
        url="/tmp/unused-test-mirror.git",
        status="pending",
...
def test_publish_request_defaults_and_zero_wait():
    assert PlaybillLedgerPublishRequest().timeout == 60
    assert PlaybillLedgerPublishRequest(timeout=0).timeout == 0
    with pytest.raises(ValidationError):
        PlaybillLedgerPublishRequest(timeout=0, url="/tmp/not-configurable-here")


def test_runtime_barrier_preserves_own_acknowledgment_with_newer_request(monkeypatch):
    state = _state()
    calls = []
...
def test_http_publish_forwards_bounded_wait_and_returns_full_status(playbill_http, monkeypatch):
