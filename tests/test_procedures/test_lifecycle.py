...
def test_propose_and_accept_pin_definition_config_and_lock_digests(
    ...
    # The get surface returns the read record and accept returns the bare
    # persisted one, so the run-ledger block is compared on its own and the
    # rest of the record must match the accepted state field for field.
    detail_payload = details.procedure.model_dump(mode="python")
    assert detail_payload.pop("track_record") == ProcedureTrackRecord().model_dump(mode="python")
    assert detail_payload == accepted.procedure.model_dump(mode="python")
