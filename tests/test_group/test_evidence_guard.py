...
def test_unevidenced_support_requires_review_when_flagged() -> None:
    ...
    assert derive_review_priority(
        members,
        _policy(require_evidence_on_support=True),
        _trusted_resolution(),
    ) == "review"


def test_support_evidenced_by_refs_does_not_require_review() -> None:
    members = [
        _member(
...
def test_support_evidenced_by_source_query_does_not_require_review() -> None:
    members = [
        _member(
...
def test_unevidenced_support_blocks_auto_resolve_when_flagged() -> None:
    ...
    assert check_auto_resolve_signals(
        members,
        _policy(require_evidence_on_support=True),
    ) is False
