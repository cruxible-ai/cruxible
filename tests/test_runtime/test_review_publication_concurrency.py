...
from cruxible_core.proposals.proposal_notes import proposal_approval_note
...
def test_stale_handle_reconciliation_cannot_resurrect_a_settled_proposal(tmp_path):
    ...
    assert "refs/settled/archive" in instance._ledger.mirror_refs()
    instance._ledger._git(["reflog", "expire", "--expire=now", "--all"])
    instance._ledger._git(["gc", "--prune=now"])
    assert instance._ledger.object_exists(proposed.admission.candidate_commit_oid)
    ...
    assert "refs/settled/archive" in stale._ledger.mirror_refs()
    assert stale._ledger.object_exists(proposed.admission.candidate_commit_oid)
