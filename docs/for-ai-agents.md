...
## Recipe: Review Handoff Preflight

For code-change work in this repository, the project-state `ReviewRequest` is
the handoff authority. Before merge or release, run the review handoff preflight
...
CRUXIBLE_CHANGE_REPO=cruxible-ai/cruxible-core \
CRUXIBLE_CHANGE_HEAD=<exact-reviewed-commit-sha> \
CRUXIBLE_EXPECTED_CONFIG_NAME=project_state \
uv run python scripts/check_review_handoff.py
```

The check passes only when the target project-state instance contains an
approved `ReviewRequest` with matching `change_head`, matching
`change_repo` when supplied, and at least one `review_request_for_work_item`
edge to a `WorkItem`.

CI uses the same script when `CRUXIBLE_STATE_URL`, `CRUXIBLE_INSTANCE_ID`, and
`CRUXIBLE_STATE_READER_TOKEN` are configured. Without those values, the public
CI job skips the state-backed check so forks and unauthenticated environments
can still run tests. Release shells should treat the script as mandatory.

Keep `CRUXIBLE_EXPECTED_CONFIG_NAME=project_state` for this repository. It
prevents an agent from accidentally checking an agent-operation or other daemon
instance whose review queue is unrelated to the code-change work.
