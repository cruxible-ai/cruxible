...
**Usage:** `cruxible review [OPTIONS]`

**Purpose:** Review request helpers.

**Subcommands:**

- `cruxible review submit` - Create a project-state `ReviewRequest` for a completed code change.
...
## cruxible review submit

**Usage:** `cruxible review submit WORK_ITEM_ID [OPTIONS]`

**Purpose:** Create a project-state `ReviewRequest` for a completed code
change, link it to the target `WorkItem`, and submit the write through the same
guarded direct-write validation path as `batch-direct-write`.

**Key Options:**

| Name | Required | Default | Type | Description |
| --- | --- | --- | --- | --- |
| `WORK_ITEM_ID` | yes | | text | Target `WorkItem` id. |
| `--review-request-id` | no | `rr-WORK_ITEM_ID-HEAD12` | text | ReviewRequest id to create. |
| `--change-repo` | no | inferred from `CRUXIBLE_CHANGE_REPO` or git remote | text | Repository under review; required if not inferable. |
| `--change-base` | no | inferred from `CRUXIBLE_CHANGE_BASE` or git merge-base | text | Base commit under review; required if not inferable. |
| `--change-head` | no | inferred from `CRUXIBLE_CHANGE_HEAD` or git `HEAD` | text | Exact reviewed commit SHA; required if not inferable. |
| `--summary` / `--summary-file` | no | generated from change refs | text/path | Implementer-owned review summary. |
| `--requested-by` | no | | text | Requester identity. |
| `--reviewer` | no | | text | Requested reviewer identity. |
| `--requested-at` | no | today | date | Request date in `YYYY-MM-DD` form. |
| `--dry-run` | no | `False` | boolean | Validate without mutating graph state. |
| `--json` | no | `False` | boolean | Emit a stable JSON envelope. |

**Output And Side Effects:**
- Validates that the target `WorkItem` exists and that the ReviewRequest id does
  not already exist.
- Creates one `ReviewRequest` entity with `status=requested` by default.
- Adds `review_request_for_work_item`.
- When the work item already has `work_item_in_release` or
  `work_item_in_milestone` links, also adds matching
  `review_request_in_release` and `review_request_in_milestone` context links.
  Release context is also carried through `milestone_in_release` when the work
  item is linked to a milestone.
- Apply mode returns the new mutation receipt; dry-run mode returns no receipt.

**Example:**

```bash
cruxible review submit wi-review-submit-command \
  --change-repo cruxible-ai/cruxible-core \
  --change-base 266d1ab5d4f1ce2b5988eba639fda9bc3f1dab12 \
  --change-head "$(git rev-parse HEAD)" \
