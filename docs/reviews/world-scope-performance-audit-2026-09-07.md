...
Batch-1 clarification from the maintainer discussion: dependency pin granularity belongs in the initial shared-contract design, before index implementations. Classify exact-revision, identity/existence, explicit semantic facet, query/range/absence and historical-evidence dependencies; audit each law's actual reads before narrowing its commitments. Existing historical pins and digest rules retain their meaning.
